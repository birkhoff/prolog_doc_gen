...
public class Compiler {
	...
	public static void main(String args[]) throws ParserException, LexerException, InterruptedException
	{
	   try
	   {
		...
		try
		{
		  String cmd = "sicstus -l codeq_analyzer --goal \"tell('foo.clj'), prolog_flag(redefine_warnings, _, off),on_exception(X,(use_module('"+filename+"'),write_clj_representation,told, halt),(print('{:error \"'),print(X),print('\"}'),nl,halt(1))).\"" ;
